...
import torch
from model.fusion01 import fusion
...
class FNet(nn.Module):
    def __init__(self, pt=PointNet(emb_dims=1024 + 64), PSFE=LocalStructureFeatureExtractor(), droput=0.0,
                 pooling='max'):
    ...
    def forward(self, template, source, maxIteration=2):
        ...
        for i in range(maxIteration):
            est_R, est_t, source = self.get_transforme(template_pt, template_feature, source, est_R, est_t)
    ...
    def get_transforme(self, template_pt, template_feature, source, est_R, est_t):
        ...
        source_pt = source_pt_f.squeeze(1)
        # self.template_pt = template_pt + template_pt_f.squeeze(1)

        source_y = torch.cat([source_pt, source_psfe], dim=1)
        y1 = self.fc5(source_y)

        # template_y = torch.cat([template_pt, template_psfe], dim=1)
        # y2 = F.relu(self.fc5(template_y))
        y = torch.cat([y1, template_feature], dim=1)
        # y = torch.cat([source_pt, template_pt], dim=1)
