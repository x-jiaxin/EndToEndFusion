...
class BaseNet(nn.Module):
    def __init__(self, pt=PointNet(emb_dims=1024), pooling='max'):
    ...
    def forward(self, template, source, maxIteration=2):
        template_feature = self.pooling(self.PT(template))  # 1024
    ...
    def get_transforme(self, template_feature, source, est_R, est_t):
        source_feature = self.pooling(self.PT(source))  # 1024
