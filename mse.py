...
def summary_metrics(r_mse, t_mse, r_mae, t_mae):
    """
    www
    ...
    r_mse, t_mse, r_mae, t_mae = np.sqrt(np.mean(r_mse)), np.sqrt(np.mean(t_mse)), np.mean(r_mae), np.mean(t_mae)
    return r_mse, t_mse, r_mae, t_mae
...
def compute_metrics(R, t, gtR, gtt):
    ...
    return cur_r_mse, cur_t_mse, cur_r_mae, cur_t_mae
